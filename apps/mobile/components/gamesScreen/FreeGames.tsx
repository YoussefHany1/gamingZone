...
import { View,
  ...
  StyleSheet,
  ScrollView,
  ToastAndroid, Image } from "react-native";
...
const FreeGameCard = React.memo<FreeGameCardProps>(({ item, onClaim, t }) => {
  ...
  // Use refs so callbacks are stable — no item object in deps
  ...
  const handleClaimPress = useCallback((): void => {
    onClaimRef.current(itemRef.current);
  }, []);
    ...
    >
      ...
      <View style={styles.imageContainer}>
        ...
        <Image
          source={
            item.image
              ? { uri: `https://wsrv.nl/?url=${item.image}&w=400&q=80&output=webp` } : require("../../assets/image-not-found.webp")
          }
          style={styles.cover}
          resizeMode="cover"
        />
...
FreeGameCard.displayName = "FreeGameCard";
...
function FreeGames(): React.ReactElement {
  ...
  // renderItem has empty deps — navigation & t live inside FreeGameCard itself
  const renderGameItem = useCallback(
    ({ item }: ListRenderItemInfo<FreeGameItem>) => (
      <FreeGameCard item={item} onClaim={handleClaimPress} t={t} />
    ),
    [handleClaimPress, t],
  );
