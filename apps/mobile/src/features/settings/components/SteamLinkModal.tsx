...
export default function SteamLinkModal({ visible, onClose }: Props) {
  ...
  const resolveSteamId = async (input: string): Promise<string | null> => {
    ...
    try {
      const resp = await axios.get(
        `${SERVER_URL}/steam/resolve?vanityurl=${encodeURIComponent(vanity)}`,
      );
  ...
  const handleSync = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        "==> Step 3: Fetching IGDB mappings for AppIDs amount:",
        appIdsToMap.length,
      );
      ...
      if (uniqueWishlistAppIds.length > 0) {
        console.log(
          "==> Step 5: Fetching IGDB mappings for wishlist amount:",
          uniqueWishlistAppIds.length,
        );
        try {
      ...
      for (const igdbG of mappedGames) {
        ...
        const steamG = gamesToProcess.find(
          (g) => String(g.appid) === String(igdbG.steam_appid),
        );
        if (!steamG) continue;
      ...
      if (totalImported === 0) {
        ToastAndroid.show(
          t("settings.profile.steam.errors.privateProfile") || "No games found",
          ToastAndroid.LONG,
        );
        setLoading(false);
        return;
      ...
      console.error(e);
      ToastAndroid.show(
        t("settings.profile.steam.errors.fetchFailed") || "Sync failed",
        ToastAndroid.LONG,
      );
      setLoading(false);
    }
