...
const EventCard = React.memo(function EventCard({
  ...
  const { status, formatEventDate, countdown } = useEventCard(item, lang);
  ...
  return (
    <Link
      href={`/events/${item.id}`}
      className={`relative block shrink-0 w-80 h-56 rounded-2xl overflow-hidden glass-panel border transition-all duration-300 group cursor-pointer ${
        status === "live"
    ...
    >
      ...
      <div className="absolute inset-0 bg-linear-to-t from-dark-bg via-dark-bg/60 to-transparent z-10"></div>
...
const GamingEvents = React.memo(function GamingEvents({
  ...
  initialEvents?: GamingEvent[];
}) {
  const { t } = useLangStore();

  const { events, loading, error } = useGamingEvents(initialEvents);
      ...
      {loading ? (
        <div className="flex gap-4 overflow-x-auto pb-4 scrollbar px-4 md:px-0">
          {[1, 2, 3].map((n) => (
            <div
              key={n}
              className="shrink-0 w-80 h-56 rounded-2xl glass-panel animate-pulse bg-white/5"
            ></div>
          ))}
        </div>
      ) : error || events.length === 0 ? (
        <Card className="rounded-2xl p-8 text-center text-gray-400 mx-4 md:mx-0 flex flex-col items-center gap-2 border-white/5">
