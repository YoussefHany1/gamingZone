        ...
        {games.map((game, index) => {
          ...
          return (
            <Link
              key={`${game.id}-${index}`}
              href={`/games/${game.id}`}
              className={`group shrink-0 ${showCountdown ? "w-56" : "w-44"} rounded-2xl overflow-hidden glass-panel glass-panel-hover border border-white/10 p-3 flex flex-col gap-3 shadow-md`}
            >
