...
const YEARS: string[] = Array.from({ length: 35 }, (_, i) => String(2025 - i));
...
const FilterSection = memo<SectionProps>(
  ({ title, items, selected, onSelect, hideAny }) => {
    const { t } = useTranslation();
    const normalizedItems: FilterOption[] =
...
function FilterModal({ visible, filters, onApply, onClose }: FilterModalProps) {
  ...
  const activeCount =
    ...
    (draft.genre ? 1 : 0) +
    (draft.platform ? 1 : 0) +
    (draft.sort && draft.sort !== "relevance" ? 1 : 0);
