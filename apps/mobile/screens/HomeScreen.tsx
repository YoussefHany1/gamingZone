...
// Each wrapper is memoized and manages its own concerns so that the parent
// renderItem function can remain completely dependency-free and stable.

/**
 * AdBanner now owns its own `showAds` state.
 * Previously this state lived in HomeScreen and was included in renderItem's
 * useCallback deps, causing every FlatList row to re-render when ads loaded.
 */
const AdBanner = memo(() => {
...
/** Stable memoized wrapper — only re-renders when language changes. */
...
// Stable no-op defined at module scope — keeps NewsSection's onChangeFeed prop
// referentially stable without any useCallback overhead.
const noop = () => {};
...
function HomeScreen(): React.ReactElement {
  ...
  // Keep a ref so renderItem can read the latest lang without being a dep.
  ...
  const sectionsData = useMemo<SectionItem[]>(() => {
    ...
    return [
      ...
      { type: "ad", _key: "ad_0" },
      { type: "news", category: "reviews", _key: `news_1_${currentLang}` },
      { type: "events", _key: "events" },
      { type: "ad", _key: "ad_1" },
      { type: "news", category: "esports", _key: `news_2_${currentLang}` },
      { type: "ad", _key: "ad_2" },
      { type: "news", category: "hardware", _key: `news_3_${currentLang}` },
    ];
  }, [currentLang]);

  /**
   * renderItem is fully stable (empty deps) — it reads the latest language
   * via currentLangRef at call-time. The NewsSection keys already include the
   * language, so React will remount the correct instance on language change.
   */
