...
export default function WeeklyRecap({ initialSummary }: WeeklyRecapProps) {
  ...
  const { summary, loading, expanded, setExpanded } =
    useWeeklyRecap(initialSummary);
  ...
  return (
    <Card className="mx-4 md:mx-0 my-10 p-6 shadow-lg relative overflow-hidden transition-all duration-300">
      {/* Background Glow */}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-4 mb-4">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-linear-to-tr from-light-blue to-secondary-blue rounded-xl text-white">
      ...
      >
        ...
        {!expanded && (
          <div className="absolute bottom-0 inset-x-0 h-12 bg-linear-to-t from-dark-bg/90 to-transparent pointer-events-none"></div>
        )}
      </div>
