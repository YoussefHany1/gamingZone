...
// دالة جلب البيانات محسّنة للعمل مع useCachedData
const createFetchGamesFunction = (endpoint, query) => {
  return async () => {
...
export default function GamesList({ endpoint, query, header }) {
  ...
  // إنشاء دالة الجلب بناءً على endpoint و query
  const fetchGames = useCallback(createFetchGamesFunction(endpoint, query), [
    endpoint,
    query,
  ]);
  ...
  const {
    ...
    isLoading,
    error,
  } = useCachedData(STORAGE_KEY, fetchGames, [endpoint, query]);
